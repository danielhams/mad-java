...
public class SpectralRollerBufferSampleFactory
	implements RollPainterSampleFactory<SpectralRollerBuffer,SpectralRollerBufferCleaner>,
	ScaleLimitChangeListener
{
	private static Log log = LogFactory.getLog( SpectralRollerBufferSampleFactory.class.getName() );
	...
	private final SpectralRollerBufferCleaner bufferClearer;
	...
	public SpectralRollerBufferSampleFactory( final SpectralRollerMadUiInstance uiInstance,
			...
			final float valueScaleForMargins )
	{
		...
		bufferClearer = new SpectralRollerBufferCleaner( displayBounds );
...
//		log.debug("Setting capture render length to " + captureRenderLength + " which is " + numSamplesPerPixel + " samples per pixel" );
	...
	@Override
	public SpectralRollerBufferCleaner getBufferClearer()
	{
		return bufferClearer;
	}

	@Override
	public SpectralRollerBuffer createBuffer( final int bufNum ) throws DatastoreException
	{
		...
		return new SpectralRollerBuffer( tbi );
	}

	@Override
	public void freeBuffer( final SpectralRollerBuffer bufferToFree ) throws DatastoreException
	{
	...
	private void fillInMinMaxLine( final Graphics2D g, final int pixelX, final float minValue, final float maxValue )
	{
		...
		g.setColor( SpectralRollerColours.DISPLAY_VALUE_COLOUR );
...
//					log.debug("Is delta update");
	...
	@Override
	public void fullFillSamples( final RollPaintUpdate update, final SpectralRollerBuffer buffer )
	{
...
//				log.debug("Pixel " + numPixelsDone + " reading from index " + indexInt );
	...
	@Override
	public void deltaFillSamples( final RollPaintUpdate update, final int displayOffset, final SpectralRollerBuffer buffer,
			final int bufferSampleOffset, final int numSamples, final SpectralRollerBuffer otherBuffer )
	{
		final Graphics2D g = buffer.graphics;
		// Nasty assumptions:
