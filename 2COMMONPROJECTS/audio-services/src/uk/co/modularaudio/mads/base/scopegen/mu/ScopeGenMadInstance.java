...
public class ScopeGenMadInstance<D extends ScopeGenMadDefinition<D, I>,
	I extends ScopeGenMadInstance<D,I>>
	extends MadInstance<D, I>
{
	private static Log log = LogFactory.getLog( ScopeGenMadInstance.class.getName() );
		...
		throws MadProcessingException
	{
		...
		backEndFrontEndBuffer = new MultiChannelBackendToFrontendDataRingBuffer( ScopeGenMadDefinition.NUM_VIS_CHANNELS, maxRingBufferingInSamples );
	...
	private int doOneTriggerPrePass( final int frameOffset,
			...
			final MadChannelBuffer[] channelBuffers )
	{
		final float[] triggerFloats = channelBuffers[ ScopeGenMadDefinition.SCOPE_TRIGGER ].floatBuffer;
			...
			final MadChannelBuffer[] channelBuffers )
	{
		...
		sourceBuffers[0] = channelBuffers[ ScopeGenMadDefinition.SCOPE_TRIGGER ].floatBuffer;
		sourceBuffers[1] = channelBuffers[ ScopeGenMadDefinition.SCOPE_INPUT_0 ].floatBuffer;
		sourceBuffers[2] = channelBuffers[ ScopeGenMadDefinition.SCOPE_INPUT_1 ].floatBuffer;
		sourceBuffers[3] = channelBuffers[ ScopeGenMadDefinition.SCOPE_INPUT_2 ].floatBuffer;
		sourceBuffers[4] = channelBuffers[ ScopeGenMadDefinition.SCOPE_INPUT_3 ].floatBuffer;
			...
			final long eventFrameTime )
	{
		...
		localBridge.queueTemporalEventToUi( tses,
				eventFrameTime,
				ScopeGenIOQueueBridge.COMMAND_OUT_RINGBUFFER_WRITE_INDEX,
				writePosition,
				null );
		workingFrontEndPeriodFramesCaptured = 0;
	}
			...
			final long eventFrameTime )
	{
		...
		localBridge.queueTemporalEventToUi( tses,
				eventFrameTime,
				ScopeGenIOQueueBridge.COMMAND_OUT_DATA_START,
				1,
				null );
	}
