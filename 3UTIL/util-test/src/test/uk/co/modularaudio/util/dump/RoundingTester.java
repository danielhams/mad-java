...
public class RoundingTester extends TestCase
{
	...
	private void adjustVal( float testVal )
	{
		log.debug( "############# Incoming float is " + MathFormatter.slowFloatPrint( testVal, 3, true ) );
		...
		log.debug("Adjusted with that, we have " + MathFormatter.slowFloatPrint( adjustedVal, 3, true ) );
		if( adjustedVal < 0.0f )
		{
			log.debug("Adjusting up one.");
			adjustedVal += 1.0f;
		}
		log.debug("So final val is " + MathFormatter.slowFloatPrint( adjustedVal, 3, true ) );
	}
