...
//		BasicConfigurator.configure();
	...
	public void testCreatingAndAddingToTable() throws ContentsAlreadyAddedException, TableCellFullException, TableIndexOutOfBoundsException, DatastoreException
	{
		...
		TableInterface<AudioComponent, AudioComponentProperties> table = new Table<AudioComponent, AudioComponentProperties>( 10, 10 );
		// Add something that is 2 by 2 at 0,0
		DJMixerComponent firstObj = new DJMixerComponent();
		table.addContentsAtPosition( firstObj, 0, 0 );
		
		Span ecCellSpan = firstObj.getCellSpan();
		for( int i = 0 ; i < ecCellSpan.x ; i++ )
		{
			for( int j = 0 ; j < ecCellSpan.y ; j++ )
			{
				AudioComponent check = table.getContentsAtPosition( i, j );
		...
		thrown = false;
		DJMixerComponent secondObj = new DJMixerComponent();
		try
		{
			table.addContentsAtPosition( secondObj, 0, 1 );
		}
		catch (TableCellFullException e)
		{
			thrown = true;
		}
		assertTrue( thrown );
		...
		TablePrinter<AudioComponent, AudioComponentProperties> printer = new TablePrinter<AudioComponent, AudioComponentProperties>();
		printer.printTableContents( table );
		
		TablePosition firstObjOrigin = table.getContentsOriginReturnNull( firstObj );
		log.debug("For the first object I get origin: " + firstObjOrigin);
		assertEquals( firstObjOrigin, new TablePosition(0,0) );
		
		TablePosition secondObjOrigin = table.getContentsOriginReturnNull( secondObj );
		...
		DJMixerComponent lastObj = new DJMixerComponent();
		thrown = false;
		try
		{
		...
		throws ContentsAlreadyAddedException, TableCellFullException, NoSuchContentsException, TableIndexOutOfBoundsException
	{
		...
		// Add something that is 2 by 2 at 0,0
		DJMixerComponent firstObj = new DJMixerComponent();
		DJMixerComponentProperties firstObjProperties = new DJMixerComponentProperties();
		table.addContentsAndPropertiesAtPosition( firstObj, firstObjProperties, 0, 0 );
		
		AudioComponentProperties getTest = table.getPropertiesAtPosition( 0, 0 );
		...
		throws ContentsAlreadyAddedException, TableCellFullException, NoSuchContentsException, TableIndexOutOfBoundsException, DatastoreException
	{
		...
		boolean thrown = false;
		DJMixerComponent nonObj = new DJMixerComponent();
		...
		AudioComponent nothing = testTable.getContentsAtPosition( 0, 5 );
		assertNull( nothing );
		TablePrinter<AudioComponent, AudioComponentProperties> printer = new TablePrinter<AudioComponent, AudioComponentProperties>();
		...
		DJMixerComponent[] objsToAdd = new DJMixerComponent[ 5 ];
		...
		TestTableListener removalListener = new TestTableListener();
		testTable.addListener( removalListener );
		
		TablePosition positionBeforeDelete = testTable.getContentsOriginReturnNull( objsToAdd[ 2 ] );
		...
		AudioComponent noSuchComponent = testTable.getContentsAtPosition( positionBeforeDelete.x, positionBeforeDelete.y );
		...
		throws ContentsAlreadyAddedException, TableCellFullException, NoSuchContentsException, TableIndexOutOfBoundsException, DatastoreException
	{
		...
		TestTableListener lis = new TestTableListener();
	...
	public void testResizeTable() throws ContentsAlreadyAddedException, TableCellFullException, TableResizeException, TableIndexOutOfBoundsException, DatastoreException
	{
		...
		TableInterface<AudioComponent, AudioComponentProperties> table = new Table<AudioComponent, AudioComponentProperties>(2, 2);
		DJMixerComponent testComponent = new DJMixerComponent();
