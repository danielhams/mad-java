...
public class LaunchTestLayeredPaneDndTable extends JFrame
{
	...
	public JComponent getFrontAudioComponentTable() throws DatastoreException
	{
		...
		{
			final GuiTableDataModel<TestTC, TestTP> tableModel = new GuiTableDataModel<TestTC, TestTP>(100, 100);
			try
			{
				final TestIOBar ioBar = new TestIOBar();
...
//				tableModel.addContentsAtPosition( contentsTwo, 0, 2 );
			...
			final Color gridColor = new Color( 80, 80, 80);
			final TestECP emptyCellPainter = new TestECP();
			final TestDndCTGF componentToGuiFactory = new TestDndCTGF();
			final TestDndRackDecorations dndDecorations = new TestDndRackDecorations();
			final TestDndRackDragPolicy dndTablePolicy = new TestDndRackDragPolicy( tableModel, dndDecorations );
			final LayeredPaneDndTable<TestTC, TestTP, TestGC> testDndTable = new LayeredPaneDndTable<TestTC, TestTP, TestGC>( tableModel,
