...
public class LimiterFast
{
	public static Log log = LogFactory.getLog( LimiterFast.class.getName() );
	...
	public LimiterFast( final double knee, final double falloff )
	{
		this.knee = knee;
	...
	public void filter( final float[] output, final int outOffset, final int length, final float[] kneeFloats,
			final int kneeTmpIndex, final float[] falloffFloats, final int falloffTmpIndex )
	{
		for( int s = 0; s < length; ++s )
		{
			...
			else if( absVal > knee )
			{
				...
				final float mappedValue = AudioMath.tanhNoClipOnlyPositive( normalisedScalor );
