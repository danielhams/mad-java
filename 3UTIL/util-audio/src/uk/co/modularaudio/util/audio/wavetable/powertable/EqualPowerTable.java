...
public class EqualPowerTable extends RawCrossfadePowerTable
{
	public EqualPowerTable( int bufferLength )
	{
		super( bufferLength );
		...
		for( int i = 0 ; i < bufferLength ; i++ )
		{
			...
			float powerLeftVal = (float)Math.sin( leftVal * (Math.PI / 2.0 ));
			leftFloatBuffer[ i ] = powerLeftVal;
			float powerRightVal = (float)Math.cos( leftVal * (Math.PI / 2.0) );
			rightFloatBuffer[ i ] = powerRightVal;
		}
