...
public class FrequencyBanderTester extends TestCase
{
	...
	{
		MidiNote startMidiNote = MidiUtils.getMidiNoteFromStringReturnNull( "C3" );
		MidiNote endMidiNote = MidiUtils.getMidiNoteFromStringReturnNull( "G10" );
		int numNotesBetweenBand = 1;
		FrequencyBander fb = new FrequencyBander( startMidiNote, endMidiNote, numNotesBetweenBand );
		int numBands = fb.getNumBands();
		float[] baseFreqPerBand = fb.getBaseFreqPerBand();
		int[] numHarmsPerBand = fb.getNumHarmsPerBand();
